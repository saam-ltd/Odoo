    ...
    def change_to_backdate(self):
            ...
            for picking in self.picking_ids:
                ...
                for acc_mv in accmoveObj:
                    ...
                    self.env.cr.execute('update account_move set create_date=%s where id=%s', (self.date, acc_mv.id))
                    acc_mv.action_post()
                    for journal_items in acc_mv.line_ids:
                        self.env.cr.execute('update account_move_line set create_date=%s where id=%s', (self.date, journal_items.id))
                ...
                for move in moveObj:
                    ...
                    self.env.cr.execute('update stock_move set create_date=%s where id=%s', (self.date, move.id))
                    ...
                    for move_line in movelineObj:
                        ...
                        self.env.cr.execute('update stock_move_line set create_date=%s where id=%s', (self.date, move_line.id))
